...
if ((!ID_USER || ID_USER === "null" || ID_USER === "undefined") || (ROLE_USER == 3 || ROLE_USER == 2 || ROLE_USER == 1)) {
  ...
  const addFournisseur = document.getElementById("addFournisseur");
  ...
  addFournisseur.addEventListener("submit", event => {
    event.preventDefault();
    const formData = new FormData(addFournisseur);
      ...
      fetch(`${BACK_URLROOT}Users/register`, {
        ...
        .then(data => {
          if (data.message == "Account Added") {
            addAlerteNonLierAuStock(6)
            location.replace(`${URLROOT}users/fournisseurs`);
          } else {
